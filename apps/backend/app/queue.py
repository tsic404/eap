...
task type (see ``QUEUE_NAME_MAP``). The transactional-outbox reconciler
(``app.services.task_reconcile``) re-enqueues undelivered outbox rows using the
same ``job_id`` so an enqueue is naturally idempotent.
"""
...
def _task_queue(queue_name: str) -> Queue:
    """Return an RQ queue for ``queue_name`` on the shared Redis connection."""
...
def enqueue_process_task(
    ...
    ``job_id`` is the outbox event id, shared with the reconciler's requeue path,
    so re-enqueueing an already-delivered outbox row is a no-op in RQ (same
    ``job_id`` never enqueues twice). ``queue`` is injectable for tests.
    """
